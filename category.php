...
$adjacents = 7;
			...
			$statement = $db->prepare("SELECT * FROM table_post WHERE cat_id=? ORDER BY post_id DESC");
			...
			$statement = $db->prepare("SELECT * FROM table_post WHERE cat_id=? ORDER BY post_id DESC LIMIT $start, $limit");
			$statement->execute(array($id));
				...
				{   
					for ($counter = 1; $counter <= $lastpage; $counter++)
					{
						if ($counter == $page)
							...
							$pagination.= "<a href=\"$targetpage?id=$id&page=$counter\">
